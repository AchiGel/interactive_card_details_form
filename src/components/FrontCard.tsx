import "../styles/frontCard.css";
import icons from "../images/card-logo.svg";

function FrontCard() {
  return (
    <div className="frontCard">
      <div className="frontCard-icons">
        <img src={icons} alt="icons" />
      </div>
      <div className="frontCard-number">
        <span className="frontCard-number-nums">0000 0000 0000 0000</span>
      </div>
      <div className="frontCard-bottom">
        <span className="frontCard-name">JANE APPLESEED</span>
        <span className="frontCard-date">00/00</span>
      </div>
    </div>
  );
}

export default FrontCard;
